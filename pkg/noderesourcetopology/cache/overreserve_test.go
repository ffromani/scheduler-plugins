...
func TestResyncReserveConcurrencyRegression(t *testing.T) {
	fakeClient, err := tu.NewFakeClient()
	if err != nil {
		t.Fatal(err)
	}
	...
	nrtUpdates := nrtCache.MakeNRTUpdatesForNodes(context.Background(), lh, nodes)
	...
	nrtObj, _ := nrtCache.GetCachedNRTCopy(context.Background(), "node1", concurrentPod)
	if nrtObj == nil {
