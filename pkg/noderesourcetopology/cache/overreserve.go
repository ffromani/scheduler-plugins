...
func (ov *OverReserve) MakeNRTUpdates(ctx context.Context, lh_ logr.Logger, nodes DesyncedNodes) []*topologyv1alpha2.NodeResourceTopology {
	...
	if part := ov.makeNRTUpdatesForNodes(ctx, lh_, ov.client, nodes.MaybeOverReserved, "resynced", isNRTFresher); len(part) > 0 {
		nrtUpdates = append(nrtUpdates, part...)
	}

	alwaysPass := func(_ logr.Logger, _ *topologyv1alpha2.NodeResourceTopology) error { return nil }
...
func (ov *OverReserve) makeNRTUpdatesForNodes(ctx context.Context, lh_ logr.Logger, rd ctrlclient.Reader, nodeNames []string, reason string, gateCheck gateFunc) []*topologyv1alpha2.NodeResourceTopology {
	var nrtUpdates []*topologyv1alpha2.NodeResourceTopology
	for _, nodeName := range nodeNames {
		...
		if err := gateCheck(lh, nrtCandidate); err != nil {
			lh.V(2).Info("failed gate", "reason", err.Error())
			continue
		}

		lh.V(4).Info("overriding cached info", "reason", reason)
