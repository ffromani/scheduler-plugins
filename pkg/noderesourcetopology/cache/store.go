...
func newNrtStore(nrts []*topologyv1alpha2.NodeResourceTopology, lh logr.Logger) *nrtStore {
...
func checkPodFingerprintForNode(lh logr.Logger, objs []podData, nodeName, pfpExpected string, onlyExclRes bool) error {
	...
	lh.V(5).Info("podset fingerprint check", "node", nodeName, "expected", pfpExpected, "computed", pfpComputed, "onlyExclusiveResources", onlyExclRes)
	lh.V(6).Info("podset fingerprint debug", "node", nodeName, "status", st.Repr())
...
func makeNodeToPodDataMap(podLister podlisterv1.PodLister) (map[string][]podData, error) {
