...
func (tm *TopologyMatch) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	tm.log.V(6).Info("scoring node", "nodeName", nodeName)
	...
	if !ok {
		tm.log.V(4).Info("noderesourcetopology is not valid for node", "node", nodeName)
		return 0, nil
	}
	if nodeTopology == nil {
		tm.log.V(5).Info("noderesourcetopology was not found for node", "node", nodeName)
		return 0, nil
	}

	nrtlog.NRT(tm.log, "noderesourcetopology found", nodeTopology)

	handler := tm.scoringHandlerFromTopologyManagerConfig(topologyManagerConfigFromNodeResourceTopology(nodeTopology))
	if handler == nil {
		return 0, nil
	}
	return handler(pod, nodeTopology.Zones, tm.log)
}
...
func scoreForEachNUMANode(requested v1.ResourceList, numaList NUMANodeList, score scoreStrategyFn, resourceToWeightMap resourceToWeightMap, lh logr.Logger) int64 {
...
func podScopeScore(pod *v1.Pod, zones topologyv1alpha2.ZoneList, scorerFn scoreStrategyFn, resourceToWeightMap resourceToWeightMap, lh logr.Logger) (int64, *framework.Status) {
	...
	allocatablePerNUMA := createNUMANodeList(zones, lh)
	finalScore := scoreForEachNUMANode(resources, allocatablePerNUMA, scorerFn, resourceToWeightMap, lh)
	lh.V(5).Info("pod scope scoring final node score", "finalScore", finalScore)
	return finalScore, nil
}

func containerScopeScore(pod *v1.Pod, zones topologyv1alpha2.ZoneList, scorerFn scoreStrategyFn, resourceToWeightMap resourceToWeightMap, lh logr.Logger) (int64, *framework.Status) {
	...
	for i, container := range containers {
		...
		contScore[i] = float64(scoreForEachNUMANode(container.Resources.Requests, allocatablePerNUMA, scorerFn, resourceToWeightMap, lh))
...
func (tm *TopologyMatch) scoringHandlerFromTopologyManagerConfig(conf TopologyManagerConfig) scoringFn {
	...
	if conf.Scope == kubeletconfig.PodTopologyManagerScope {
		return func(pod *v1.Pod, zones topologyv1alpha2.ZoneList, lh logr.Logger) (int64, *framework.Status) {
			return podScopeScore(pod, zones, tm.scoreStrategyFunc, tm.resourceToWeightMap, lh)
		}
	}
	if conf.Scope == kubeletconfig.ContainerTopologyManagerScope {
		return func(pod *v1.Pod, zones topologyv1alpha2.ZoneList, lh logr.Logger) (int64, *framework.Status) {
			return containerScopeScore(pod, zones, tm.scoreStrategyFunc, tm.resourceToWeightMap, lh)
		}
	}
	return nil // cannot happen
}
