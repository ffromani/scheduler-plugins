...
func singleNUMAContainerLevelHandler(pod *v1.Pod, zones topologyv1alpha2.ZoneList, nodeInfo *framework.NodeInfo, lh logr.Logger) *framework.Status {
	lh.V(5).Info("Single NUMA node handler")

	// prepare NUMANodes list from zoneMap
	nodes := createNUMANodeList(zones, lh)
	...
	for _, initContainer := range pod.Spec.InitContainers {
		...
		lh_ := lh.WithValues("logID", logID)
		lh_.V(6).Info("target resources", stringify.ResourceListToLoggable("initcnt", initContainer.Resources.Requests)...)

		_, match := resourcesAvailableInAnyNUMANodes(lh_, nodes, initContainer.Resources.Requests, qos, nodeInfo)
		if !match {
			// we can't align init container, so definitely we can't align a pod
			lh_.V(2).Info("cannot align container", "name", initContainer.Name, "kind", "init")
	...
	for _, container := range pod.Spec.Containers {
		...
		lh_ := lh.WithValues("logID", logID)
		lh_.V(6).Info("target resources", stringify.ResourceListToLoggable("appcnt", container.Resources.Requests)...)

		numaID, match := resourcesAvailableInAnyNUMANodes(lh_, nodes, container.Resources.Requests, qos, nodeInfo)
		if !match {
			// we can't align container, so definitely we can't align a pod
			lh_.V(2).Info("cannot align container", "name", container.Name, "kind", "app")
		...
		subtractFromNUMA(nodes, numaID, container, lh)
	}
	return nil
}
...
func resourcesAvailableInAnyNUMANodes(lh logr.Logger, numaNodes NUMANodeList, resources v1.ResourceList, qos v1.PodQOSClass, nodeInfo *framework.NodeInfo) (int, bool) {
	...
	for resource, quantity := range resources {
		if quantity.IsZero() {
			...
			lh.V(4).Info("ignoring zero-qty resource request", "node", nodeName, "resource", resource)
			continue
		}
		...
		if _, ok := nodeResources[resource]; !ok {
			...
			lh.V(5).Info("early verdict: cannot meet request", "node", nodeName, "resource", resource, "suitable", "false")
			return numaID, false
		}
		...
		for _, numaNode := range numaNodes {
			...
			lh.V(6).Info("feasible", "node", nodeName, "NUMA", numaNode.NUMAID, "resource", resource)
		}
		...
		if !hasNUMAAffinity && (!v1helper.IsNativeResource(resource) || resource == v1.ResourceEphemeralStorage) {
			lh.V(6).Info("resource available at node level (no NUMA affinity)", "node", nodeName, "resource", resource)
			continue
		}

		bitmask.And(resourceBitmask)
		if bitmask.IsEmpty() {
			lh.V(5).Info("early verdict", "node", nodeName, "resource", resource, "suitable", "false")
			return numaID, false
		}
	}
	...
	ret := !bitmask.IsEmpty()
	lh.V(5).Info("final verdict", "node", nodeName, "suitable", ret)
	return numaID, ret
}
...
func singleNUMAPodLevelHandler(pod *v1.Pod, zones topologyv1alpha2.ZoneList, nodeInfo *framework.NodeInfo, lh logr.Logger) *framework.Status {
	lh.V(5).Info("Pod Level Resource handler")

	resources := util.GetPodEffectiveRequest(pod)

	logID := fmt.Sprintf("%s/%s", pod.Namespace, pod.Name)
	lh = lh.WithValues("logID", logID)

	nodes := createNUMANodeList(zones, lh)

	// Node() != nil already verified in Filter(), which is the only public entry point
	logNumaNodes(lh, "pod handler NUMA resources", nodeInfo.Node().Name, nodes)
	lh.V(6).Info("target resources", stringify.ResourceListToLoggable("pod", resources)...)

	if _, match := resourcesAvailableInAnyNUMANodes(lh, createNUMANodeList(zones, lh), resources, v1qos.GetPodQOS(pod), nodeInfo); !match {
...
func (tm *TopologyMatch) Filter(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	...
	if !ok {
		tm.log.V(2).Info("invalid topology data", "node", nodeName)
	...
	tm.log.V(5).Info("Found NodeResourceTopology", "nodeTopology", nodeTopology.Name)

	handler := filterHandlerFromTopologyManagerConfig(topologyManagerConfigFromNodeResourceTopology(nodeTopology))
	if handler == nil {
		return nil
	}
	status := handler(pod, nodeTopology.Zones, nodeInfo, tm.log)
	if status != nil {
...
func subtractFromNUMA(nodes NUMANodeList, numaID int, container v1.Container, lh logr.Logger) {
